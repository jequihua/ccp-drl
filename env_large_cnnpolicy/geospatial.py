...
def distance_polys_table(polygons_gp, k=0, dropzeros = False, decimals = 2):
    ...
    distance_matrix = distance_matrix.where(np.triu(np.ones(distance_matrix.shape),
                                                    k=k).astype(bool)).stack().reset_index()
...
def conindex(paths_df, area_matrix):
    path_areas = area_matrix * paths_df.to_numpy()
    index_value = path_areas.sum()
    return index_value
...
def conefor(polygons_area_matrix,
    ...
    if os.path.isfile('results_all_EC(IIC).txt'):
        ...
        pass
    if os.path.isfile('results_all_EC(PC).txt'):
        os.remove('results_all_EC(PC).txt')
    else:
        pass
    return index
